...
SERVER_URL = "http://10.0.0.100:8000/query"  # AI server endpoint
...
def query_model(prompt):
    """Queries the remote AI server for a response."""
...
class ChatSession:
    ...
    def get_single_response(self, context):
        """Fetches a single response from the AI server."""
        response = query_model(context)
        timestamp = datetime.now().isoformat()
        self.history.append({'timestamp': timestamp, 'role': 'user', 'content': context})
        self.history.append({'timestamp': timestamp, 'role': 'assistant', 'content': response})
        self.save_history()
        return response
